...
const Page = () => {
    ...
    const [userRole, setUserRole] = useState<string>("user"); // Add state for user role
    ...
    const getUserData = async () => {
        try {
            ...
            console.log(data);
            setUserData(data);
            setUserRole(data?.userData?.role || "user"); // Set user role
            setLoading(false);
            return data;
        } catch (error) {
            setLoading(false);
        ...
        <section className="flex flex-col gap-3 pt-2">
            {userRole === "driver" ? (
                <div>
                    ...
                    {/* ...other driver-specific components... */}
                </div>
            ) : (
                <>
                    {user ? (
