...
import { Home, MapPin, Scan, Ticket, Truck, User } from "lucide-react";
import Link from "next/link";
import { usePathname } from "next/navigation";

const DriverNavbar = ({ role }: { role: string }) => {
        ...
        <nav className="fixed z-50 bottom-0 m-auto max-w-[800px] md:max-w-[400px] left-0 right-0 bg-white w-full">
            <div className="flex items-center p-4 gap-3 justify-between bg-white shadow-2xl shadow-black w-full">
                ...
                <div className="flex gap-12 items-center">
                    ...
                    {role === "driver" ? (
                        <Link href={"/driver"} className="flex justify-center gap-1 items-center flex-col">
                            <Truck size={32} />
                            <span className="text-xs font-medium opacity-70">Driver</span>
                        </Link>
                    ) : (
